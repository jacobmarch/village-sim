...
class Organization:
    ...
    def is_valid(self) -> bool:
        return len(self.creatures) >= 2 and len(self.structures) >= 1 and self.leader in self.creatures
    ...
    def __str__(self) -> str:
        return f"Organization: {self.name} (Leader: {self.leader.name}, Members: {len(self.creatures)}, Structures: {len(self.structures)}, Days Existed: {self.days_existed}, Leader Days in Charge: {self.leader_days_in_charge})"
